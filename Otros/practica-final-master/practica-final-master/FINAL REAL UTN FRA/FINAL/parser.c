...
int parserProducto(char* path , ArrayList* this)
{
    ...
    int cantidad;
    int contador=0;
    EProducto* auxiliarCliente; //--->utilizo la misma estructura para los dos depositos (EProducto y EProductoDep1)
       ...
       do
        {
            if(fscanf(pFile,"%[^,],%[^,],%[^\n]\n",bProducto,bDescripcion,bCantidad)==3)
            {
                ...
                al_add(this,auxiliarCliente);//--->Relacion ArrayList con Estructura.
                contador++;
            }
            else
